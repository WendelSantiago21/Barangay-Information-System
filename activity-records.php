...
.hero {
  ...
  padding: 30px;
  border-radius: 8px;
  width: 400px; /* or increase to 450px or 500px if you want more width */
  text-align: left;
...
$currentMonth = date('m'); // Get current month as 01, 02, ..., 12
$currentYear = date('Y');

// Clear activities if it's the 1st of the month
if (date('j') == 1) {
    $clearQuery = "DELETE FROM tblactivity WHERE MONTH(date) < ? AND YEAR(date) <= ?";
    $stmtClear = mysqli_prepare($link, $clearQuery);
    mysqli_stmt_bind_param($stmtClear, "ii", $currentMonth, $currentYear);
    mysqli_stmt_execute($stmtClear);
    mysqli_stmt_close($stmtClear);
}

// Fetch activities for current month
$query = "SELECT * FROM tblactivity WHERE MONTH(date) = ? AND YEAR(date) = ? ORDER BY date DESC";
$stmt = mysqli_prepare($link, $query);
...
while ($row = mysqli_fetch_assoc($result)) {
    echo "<div style='width: 300px; border: 1px solid #ccc; padding: 10px; text-align: center;'>";
    echo "<img src='uploads/{$row['image']}' style='width:100%; height:200px; object-fit:cover;'><br>";
...
<script>
    ...
    .then(response => {
        ...
        // Redirect only if the upload was successful
        if (response.includes("successfully")) {
            window.location.href = "activity-records.php";
        } else {
            form.reset();
